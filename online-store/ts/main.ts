    ...
    .then((data: IProduct) => {
      ...
      (searchInput as HTMLInputElement).addEventListener('input', (e) => {
        ...
        const ary = productsRaw.filter((product) =>
          ((product as unknown) as IProduct).name.toLowerCase().includes(value)
        );
        renderProducts((ary as unknown) as Array<IProduct>);
      });
        ...
        checkbox.addEventListener('click', () => {
          ...
          const ary: HTMLElement[] = productsRaw.filter((product) => {
            return Object.keys(filters).every((filter) => {
              return ((filters as unknown) as string)[(filter as unknown) as number].includes(
                ((product as unknown) as string)[(filter as unknown) as number]
              );
            });
          });
          renderProducts((ary as unknown) as Array<IProduct>);
