...
function dualInputRange() {
  ...
  inputPrice.forEach((input) => {
    input.addEventListener('input', (e) => {
      ...
      if (
        ...
        maxPrice <= (((rangeInputPrice[1] as HTMLInputElement).max as unknown) as number)
      ) {
        if ((e.target as HTMLElement).className === 'input-min') {
          (((rangeInputPrice[0] as HTMLInputElement).value as unknown) as number) = minPrice;
          (rangePrice as HTMLElement).style.left =
            (minPrice / (((rangeInputPrice[0] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
        } else {
          (((rangeInputPrice[1] as HTMLInputElement).value as unknown) as number) = maxPrice;
          (rangePrice as HTMLElement).style.right =
            100 - (maxPrice / (((rangeInputPrice[1] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
        }
      }
    });
  });
  rangeInputPrice.forEach((input) => {
    input.addEventListener('input', (e) => {
      ...
      if (maxVal - minVal < priceGap) {
        ...
        (rangePrice as HTMLElement).style.left =
          (minVal / (((rangeInputPrice[0] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
        (rangePrice as HTMLElement).style.right =
          100 - (maxVal / (((rangeInputPrice[1] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
      }
    });
  });
  // Stock
  ...
  inputStock.forEach((input) => {
    input.addEventListener('input', (e) => {
      ...
      if (
        maxPrice - minPrice >= priceGap &&
        maxPrice <= (((rangeInputStock[1] as HTMLInputElement).max as unknown) as number)
      ) {
        if ((e.target as HTMLElement).className === 'input-min') {
          (((rangeInputStock[0] as HTMLInputElement).value as unknown) as number) = minPrice;
          (rangeStock as HTMLElement).style.left =
            (minPrice / (((rangeInputStock[0] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
        } else {
          (((rangeInputStock[1] as HTMLInputElement).value as unknown) as number) = maxPrice;
          (rangeStock as HTMLElement).style.right =
            100 - (maxPrice / (((rangeInputStock[1] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
        }
      }
    });
  });
  rangeInputStock.forEach((input) => {
    input.addEventListener('input', (e) => {
      ...
      if (maxVal - minVal < priceGap) {
        ...
        (rangeStock as HTMLElement).style.left =
          (minVal / (((rangeInputStock[0] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
        (rangeStock as HTMLElement).style.right =
          100 - (maxVal / (((rangeInputStock[1] as HTMLInputElement).max as unknown) as number)) * 100 + '%';
      }
    });
  });
}
